...
export default function NavBar() {
  ...
  // Chunk jobs and employer data into columns
  const jobColumns = chunkArray(jobs, 5);
  const employerEntries = Object.entries(employmentData);
  const employerColumns = chunkArray(employerEntries, 5);
          ...
          if (label === 'Find a Job') {
            return (
              <li key={href} className='relative group'>
                ...
                <div className='absolute left-0 bg-white border rounded-lg shadow-lg hidden group-hover:flex z-50 py-4'>
          ...
          if (label === 'Employers') {
            return (
              <li key={href} className='relative group'>
                ...
                <div className='absolute left-0 bg-white border rounded-lg shadow-lg hidden group-hover:flex z-50 py-4'>
                  {employerColumns.map((column, colIndex) => (
                    <ul key={colIndex} className='min-w-[220px]'>
                      {column.map(([slug, data]) => (
                        <li key={slug}>
                          <Link
                            href={`/employers/${slug}`}
                            className='block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 whitespace-nowrap'
